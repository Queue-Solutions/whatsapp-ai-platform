...
describe('grounded reply strategy', () => {
  ...
  it('recognizes explicit requests and complaints in English and Arabic without paid calls',async()=>{
    ...
    for(const text of ['My order arrived damaged','You sent me the wrong item','My bracelet arrived broken and I want a refund','الطلب وصل غلط','عايز اشتكي','الخدمة سيئة'])expect(await strategy.reply({...context,text})).toMatchObject({action:'clarify',followUp:{state:'collecting'},reason:'complaint'});
    for(const text of ['I want a refund','I want to return a bracelet','What is your refund policy?','I want refund policy details','What are your delivery times?','If my order arrives damaged, what is the return policy?',"I do not want a refund",'عايز أرجع إسورة','ايه سياسة الاسترجاع؟'])expect((await strategy.reply({...context,text})).reason).not.toBe('complaint');
