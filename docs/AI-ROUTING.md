...
3. The semantic classifier reads the latest message, bounded history, FAQ questions and the approved branch name/city catalog. It returns strict JSON containing intent, confidence, corrected query, language, product, requested branch detail and branch labels. For nearest-branch requests it also returns the exact location words copied from the latest message plus a corrected standalone geocoding query. It receives no FAQ answers and cannot send a message.
4. The backend validates every returned branch label against the current tenant-scoped catalog. High-confidence workflow intents are executed locally: human follow-up, complaints, approved links, careers, repairs, social replies and exact branch records. Straightforward return, exchange, cancellation and refund requests are a separate policy intent: they are restricted to the approved returns FAQ and do not open personal follow-up. Complaint escalation is reserved for a reported negative incident, an explicit complaint or an unresolved earlier attempt.
   Native `geo:` coordinates and approved Google Maps URLs are always passed verbatim to the deterministic location resolver. A classifier-extracted typed area is accepted only when its evidence is present in the latest customer message and its corrected name remains textually similar; a trusted geocoder must still resolve exactly one matching settlement. The model never supplies coordinates or calculates distance.
