...
function namedBranch(text:string,sources:KnowledgeSource[]){
  const t=normalizeIntent(text);
  return sources.some(s=>{try{const d=JSON.parse(s.content);const name=String(d.value?.name??'');const words=normalizeIntent(name).match(/[\p{L}\p{N}]+/gu)??[];return d.category==='branch'&&words.some(word=>word.length>2&&!['iram','ارم','ايرام','branch','mall','btc','tjh','the'].includes(word)&&t.split(/[^\p{L}\p{N}]+/u).includes(word));}catch{return false;}});
}
export function branchScope(context:MessageContext,sources:KnowledgeSource[]=[]):'none'|'detail'|'directory'{
  ...
  if(namedBranch(text,sources))return 'detail';
