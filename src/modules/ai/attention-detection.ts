...
export function detectAttention(text: string): 'human_requested' | 'complaint' | null {
  ...
  if (!contextualComplaint && (/\b(?:my (?:order|item|product|delivery).{0,45}(?:damaged|broken|wrong|missing|never arrived|not arrived)|my .{1,35}(?:arrived|came).{0,20}(?:damaged|broken|wrong|missing)|(?:received|sent me|delivered).{0,20}(?:wrong|damaged|broken)|(?:terrible|awful|bad|poor|rude) (?:service|experience|staff)|(?:make|file|raise).{0,15}complaint|still (?:not fixed|not resolved|waiting))\b/i.test(normalized)
    || /(?:عايز|عاوز|اريد).{0,20}(?:اشتكي|اقدم شكوى)|(?:الطلب|الاوردر|المنتج).{0,30}(?:مكسور|تالف|غلط|موصلش|ماوصلش)|(?:خدمه|خدمة|معامله|معاملة).{0,15}(?:سيئه|سيئة|وحشه|وحشة)|(?:محدش|ماحدش).{0,15}بيرد/.test(normalized))) return 'complaint';
