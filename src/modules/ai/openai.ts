...
export function buildRequest(context: MessageContext, sources: KnowledgeSource[], coverage: KnowledgeCoverage = { omittedSourceCount: 0, branchDirectoryComplete: true }, recoveryReason?:string, recoveryAttempt=0) {
...
export interface ModelProvider { complete(request: string, options?:{timeoutMs?:number}): Promise<{ decision: ModelDecision; input: number; output: number }> }
export class OpenAiProvider implements ModelProvider {
  constructor(private key: string, private fetcher: typeof fetch = fetch) {}
  async complete(request: string,options?:{timeoutMs?:number}) {
