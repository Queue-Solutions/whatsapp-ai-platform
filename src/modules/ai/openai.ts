...
const instructions = `House style: friendly, warm and professional WhatsApp replies. Write IRAM as ارم in Arabic, never إيرام or إرم. Do not use semicolons, including Arabic ؛. Use short paragraphs separated by blank lines. Use at most one or two tasteful emojis. Only when replyScope is directory may you list multiple branches. For detail answer only the requested detail, without a directory. For none never list branches or suggest contacting branches. For multiple branches, put each branch in its own branchLines entry with its name and concise confirmed details. The text field should then contain only a brief introduction or closing question, without repeating those branches. Never put several branches side by side in one sentence. For other replies branchLines must be empty. Lists of services or steps should have one item per line. Keep all replies concise and within the output budget.
...
export function buildRequest(context: MessageContext, sources: KnowledgeSource[], coverage: KnowledgeCoverage = { omittedSourceCount: 0, branchDirectoryComplete: true }) {
  const body = { model: AI_MODEL, store: false, temperature: 0, max_output_tokens: MAX_OUTPUT_TOKENS,
    ...
    input: JSON.stringify({ replyScope:branchScope(context,sources), customerMessage: context.text,
      history: context.history ?? [], knowledgeCoverage: coverage, approvedSources: sources.map(s => ({ label: s.label, content: s.content })) }),
    text: { format: { type: 'json_schema', name: 'business_reply', strict: true, schema: {...outputSchema,properties:{...outputSchema.properties,branchLines:{...outputSchema.properties.branchLines,maxItems:branchScope(context,sources)==='directory'?40:branchScope(context,sources)==='detail'?1:0}}} } },
  };
