...
export const modelDecisionSchema = z.object({
  action: z.enum(['answer','clarify','unavailable','handoff','complaint','knowledge_gap']),
...
const outputSchema = { type: 'object', properties: {
  action: { type: 'string', enum: ['answer','clarify','unavailable','handoff','complaint','knowledge_gap'] },
  summary: { type: 'string', maxLength: 240 },
  branchLines: { type: 'array', items: { type: 'string' } },
...
const instructions = `House style: friendly, warm and professional WhatsApp replies. Write IRAM as ارم in Arabic, never إيرام or إرم. Do not use semicolons, including Arabic ؛. Use short paragraphs separated by blank lines. Use at most one or two tasteful emojis. For multiple branches, put each branch in its own branchLines entry with its name and concise confirmed details. The text field should then contain only a brief introduction or closing question, without repeating those branches. Never put several branches side by side in one sentence. For other replies branchLines must be empty. Lists of services or steps should have one item per line. Keep all replies concise and within the output budget.
...
export function buildRequest(context: MessageContext, sources: KnowledgeSource[], coverage: KnowledgeCoverage = { omittedSourceCount: 0, branchDirectoryComplete: true }) {
  const body = { model: AI_MODEL, store: false, temperature: 0, max_output_tokens: MAX_OUTPUT_TOKENS,
    instructions: instructions + '\nThe approved sources may be a relevant subset of the business knowledge. Consult knowledgeCoverage: when omittedSourceCount is positive, missing information may exist outside this selection; ask one clarification if the request is ambiguous; for a clear business question the selection cannot answer, return knowledge_gap and describe only the limitation of the supplied sources. Never claim a partial list is exhaustive. When branchDirectoryComplete is false, present supplied branches only as examples and ask which area the customer needs. If it is true, you may list all supplied branch records. These coverage flags do not authorize any additional business facts.' + (replyLanguage(context.text ?? '') === 'ar'
    ...
    input: JSON.stringify({ customerMessage: context.text,
      history: context.history ?? [], knowledgeCoverage: coverage, approvedSources: sources.map(s => ({ label: s.label, content: s.content })) }),
