...
export function buildRequest(context: MessageContext, sources: KnowledgeSource[], coverage: KnowledgeCoverage = { omittedSourceCount: 0, branchDirectoryComplete: true }) {
  const body = { model: AI_MODEL, store: false, temperature: 0, max_output_tokens: MAX_OUTPUT_TOKENS,
    instructions: instructions + '\nThe approved sources may be a relevant subset of the business knowledge. Consult knowledgeCoverage: when omittedSourceCount is positive, missing information may exist outside this selection; ask a concise clarifying question if the selection cannot answer the request. Never claim a partial list is exhaustive. When branchDirectoryComplete is false, present supplied branches only as examples and ask which area the customer needs. If it is true, you may list all supplied branch records. These coverage flags do not authorize any additional business facts.' + (replyLanguage(context.text ?? '') === 'ar'
