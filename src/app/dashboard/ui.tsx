...
export function Editor({ db }: { db: SupabaseClient }) {
  ...
  async function save(kind: 'faq' | 'branch', id: string, publish: boolean) {
    ...
    try {
      ...
      if (scope.current === generation) { setDirty(s => { const next = new Set(s); next.delete(id); return next; }); setNotice(publish ? 'Saved and approved for assistant use.' : 'Draft saved. This entry is not approved for assistant use.'); }
