...
export function Editor({ db }: { db: SupabaseClient }) {
  ...
  const [busy, setBusy] = useState(''); const [dirty, setDirty] = useState<Set<string>>(new Set());
  ...
  async function save(kind: 'faq' | 'branch', id: string, publish: boolean) {
    if (!canEdit || busy) return; setFeedbackId(id); setBusy(id); setError(''); setNotice('');
    const generation = scope.current;
    try {
      if (kind === 'faq') { const result = await editor.saveFaq(tenant, locale, faqs.find(f => f.id === id)!, publish); if (scope.current === generation) setFaqs(all => all.map(f => f.id === id ? result : f)); }
      else { const result = await editor.saveBranch(tenant, locale, branches.find(b => b.id === id)!, publish); if (scope.current === generation) setBranches(all => all.map(b => b.id === id ? result : b)); }
      if (scope.current === generation) { setDirty(s => { const next = new Set(s); next.delete(id); return next; }); setNotice(publish ? 'Saved and approved for future assistant use.' : 'Draft saved. This entry is not approved for assistant use.'); }
  ...
  return <main className="content"><div className="page-title"><div><div className="eyebrow">TEACH YOUR ASSISTANT</div><h1>Business knowledge</h1><p className="intro">You know your business. Give your assistant the details.</p></div><div className="account-actions"><button className="text-button" disabled={!!busy} onClick={() => setPasswordOpen(v => !v)}>Password</button><button className="text-button" disabled={!!busy} onClick={async () => { if (discard()) { await db.auth.signOut({ scope: 'local' }); } }}>Sign out</button></div></div>{passwordOpen && <PasswordForm db={db} onDone={() => setPasswordOpen(false)} />}
    ...
    <div className="guidance"><span className="guidance-icon" aria-hidden="true">i</span><div><strong>Start with what you know.</strong><p>Answers are blank for you to complete. Save your progress as a draft, then approve an entry when it is ready. Blank answers are never approved. AI replies will be connected in a later step.</p></div></div>
...
function SaveActions({ disabled, saving, published, onSave }: { disabled: boolean; saving: boolean; published: boolean; onSave: (publish: boolean) => void }) {
  return <div className="save-actions"><span>{published ? 'Saving as draft removes this entry from assistant use.' : 'Drafts stay out of assistant answers.'}</span><button className="secondary" disabled={disabled} onClick={() => onSave(false)}>{saving ? 'Saving…' : 'Save draft'}</button><button className="primary" disabled={disabled} onClick={() => onSave(true)}>Save & approve</button></div>;
}
