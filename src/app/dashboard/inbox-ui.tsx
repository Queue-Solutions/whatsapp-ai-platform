...
import {InboxRepository,attentionReason,waitingLabel,type AttentionAction,type Conversation,type InboxCounts,type InboxFilter,type InboxMessage,type InboxEvent} from '@/modules/admin/inbox';
...
const stateLabels={none:'Assistant on',waiting:'Needs you',in_progress:'Replying personally',resolved:'Resolved · Assistant paused'};
    ...
    {loading?<p role="status">Loading conversations…</p>:!memberships.length?<p className="notice">Your account needs business access.</p>:<div className="inbox-grid">
      <nav className="conversation-list" aria-label="Conversations"><div className="list-heading">{filters.find(f=>f.id===filter)!.label}<span>{counts[filter]}</span></div>
        ...
        {conversations.map(c=><button key={c.id} aria-current={c.id===selected?'true':undefined} disabled={working} onClick={()=>{if(c.id===selected)return;if(discard()){setDraftDirty(false);setSelected(c.id);}}}>
          <strong>{c.name}</strong><span className={`mode-label ${c.attention_state!=='none'?'human':''}`}>{stateLabels[c.attention_state]}</span>
...
function ConversationPanel({repository,tenant,conversation:c,canEdit,onRefresh,onDraft,onWorking}:{repository:InboxRepository;tenant:string;conversation:Conversation;canEdit:boolean;onRefresh:()=>void;onDraft:(dirty:boolean)=>void;onWorking:(busy:boolean)=>void}){
  ...
  async function action(value:AttentionAction){
    ...
    if(clearsDraft&&text&&!window.confirm('Discard this unsent reply and continue?'))return;
    setBusy(true);onWorking(true);setError('');setNotice('');
    try{
      await repository.attention(c,value);
      if(clearsDraft){setText('');setAttempt(null);onDraft(false);}
      setNotice(({reply:'You’re replying personally. The assistant stays paused. A reply already being sent may still arrive.',resolve:'Resolved. The assistant stays paused. A new customer message will bring this conversation back to Needs attention.',resume:'Assistant resumed for new messages. Earlier messages will not be replayed.',flag:'Added to Needs attention. The assistant is paused.',complaint:'Marked as a complaint. The assistant is paused.',remove_complaint:'Complaint label removed. The assistant mode has not changed.'})[value]);
  ...
  return <section className="conversation-panel" aria-label={`Conversation with ${c.name}`}>
    <div className="conversation-header"><div><h2>{c.name}</h2><span className={`mode-label ${c.automation_mode==='human'?'human':''}`}>{stateLabels[c.attention_state]}</span>{c.is_complaint&&<span className="complaint-chip">Complaint</span>}</div>
    ...
    <div className={`attention-detail ${c.attention_state==='resolved'?'is-resolved':''}`}>
      {c.attention_state!=='none'&&<div><strong>{attentionReason(c.attention_reason)}</strong><p>{c.attention_summary||'You’re handling this conversation personally.'}</p><span>{c.attention_state==='waiting'?waitingLabel(c.attention_since,now):c.attention_state==='resolved'?`Resolved ${formatTime(c.resolved_at!)}`:'In progress · You’re replying personally'}</span></div>}
    ...
    {c.attention_state==='resolved'&&<p className="resolved-note">This issue is marked resolved. The assistant stays paused until you return the conversation to it.</p>}
