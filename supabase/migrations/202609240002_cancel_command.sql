...
-- Forward-compatible support for the customer resume marker. Keeping this here
-- makes the Cancel command safe to deploy even if the preceding optional
-- customer-resume migration was not applied to an existing development database.
alter table public.message_jobs add column if not exists customer_resume boolean not null default false;
...
-- Ingestion, cancellation, epoch invalidation and auditing are one transaction.
-- Only the exact standalone word Cancel is a command; casing is ignored.
